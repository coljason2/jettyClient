...
public class ChineseCalendar {
	...
	public static Map<Integer, int[]> getZodiacMap(int year, int month, int day) {
		...
		ImmutableMap<Integer, int[]> map = new ImmutableMap.Builder<Integer, int[]>()
				...
				.put((11 + animalIndex) % 12, new int[] { 2, 14, 26, 38 }).build();
	...
	public static boolean IsTodayChineseNewYear(int year, int month, int day) {
		...
		c.computeSolarTerms();
		System.out.println(
				String.valueOf(chineseYear) + " " + String.valueOf(chineseMonth) + " " + String.valueOf(chineseDate));
	...
	public static void main(String[] arg) {
		...
		int flagg = 0;
		while (true) {
			System.out.println(Integer.toBinaryString(chineseMonths[flagg]) + " "
					+ Integer.toBinaryString(chineseMonths[flagg + 1]));
	...
	private static char[] chineseMonths = {
			// Chinese month map, 2 bytes per year, from 1900 to 2100, 402
			// bytes.
	...
	private String[] getYearTable() {
		...
		table[1] = getTextLine(27, "Chinese Calendar Year: " + (chineseYear + 1) + " ("
				+ stemNames[(chineseYear + 1 - 1) % 10] + "-" + branchNames[(chineseYear + 1 - 1) % 12] + ")");
		int ln = 2;
		String[] mLeft = null;
		String[] mRight = null;
		for (int i = 1; i <= 6; i++) {
